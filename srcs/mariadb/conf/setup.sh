...
mysql -e "CREATE DATABASE IF NOT EXISTS ${MYSQL_DATABASE};" -u root -p${MYSQL_ROOT_PASSWORD}
mysql -e "CREATE USER IF NOT EXISTS ${MYSQL_USER}@'localhost' IDENTIFIED BY '${MYSQL_PASSWORD}';" -u root -p${MYSQL_ROOT_PASSWORD}
mysql -e "GRANT ALL PRIVILEGES ON ${MYSQL_DATABASE}.* TO ${MYSQL_USER}@'%' IDENTIFIED BY '${MYSQL_PASSWORD}';" -u root -p${MYSQL_ROOT_PASSWORD}
